...
describe("range format parsing", () => {
  test("single-line shorthand (no ..)", async () => {
...
describe("insert-after (+) semantics", () => {
  ...
  test("insert multiple lines after an anchor", async () => {
    ...
    const result = await edit({
      file_path: path,
      checksum: cs,
      edits: [{ range: `+1:${h1}`, content: ["x", "y", "z"] }],
    });
  ...
  test("insert into empty file via +0:", async () => {
    ...
    const result = await edit({
      ...
      edits: [{ range: "+0:", content: ["first line", "second line"] }],
    });
...
describe("multi-edit batches", () => {
  ...
  test("adjacent ranges (non-overlapping) succeed", async () => {
    ...
    const result = await edit({
      ...
      checksum: cs,
      edits: [
        { range: `1:${h1}..2:${h2}`, content: ["AA", "BB"] },
        { range: `3:${h3}..4:${h4}`, content: ["CC", "DD"] },
      ],
    });
...
describe("content growth and shrinkage", () => {
  test("replace one line with many (file grows)", async () => {
    ...
    const result = await edit({
      file_path: path,
      checksum: cs,
      edits: [{ range: `2:${h2}`, content: ["x1", "x2", "x3", "x4", "x5"] }],
    });
...
describe("unicode and special content", () => {
  ...
  test("empty string in content array produces empty line", async () => {
...
describe("no-op detection", () => {
  ...
  test("replacing multi-line range with identical content is a no-op", async () => {
    ...
    const result = await edit({
      file_path: path,
      checksum: cs,
      edits: [{ range: `1:${h1}..3:${h3}`, content: ["aaa", "bbb", "ccc"] }],
    });
...
describe("boundary hash verification", () => {
  test("wrong start hash rejected", async () => {
    ...
    const result = await edit({
      file_path: path,
      checksum: cs,
      edits: [{ range: "1:zz..2:" + lineHash("bbb"), content: ["x", "y"] }],
    });
  ...
  test("wrong end hash rejected", async () => {
    ...
    const result = await edit({
      file_path: path,
      checksum: cs,
      edits: [{ range: lineHash("aaa") ? `1:${lineHash("aaa")}..2:zz` : "1:aa..2:zz", content: ["x", "y"] }],
    });
